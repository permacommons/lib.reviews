...
Model.prototype._promisesReady = function() {
  ...
  var verifyAll = function() {
    ...
    .then(function() {
      var allFulfilled = self._pendingPromises.every(function(promise) {
        return promise && promise._state === 'fulfilled';
      });
      return allFulfilled ? Promise.resolve() : verifyAll();
    });
  };
...
Model.prototype._waitFor = function(promise) {
  ...
  var tracked = Promise.resolve(promise).then(function(value) {
    tracked._state = 'fulfilled';
    return value;
  }).catch(function(err) {
    tracked._state = 'rejected';
    throw err;
  });
  tracked._state = 'pending';
  this._pendingPromises.push(tracked);
