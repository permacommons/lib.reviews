...
- **Expand manifest-driven inference**
  - [x] Derive relation result types directly from manifest relation metadata so models no longer need manual `types.virtual().returns<…>()` placeholders. This would also resolve the circular type inference issue where cross-model virtual fields (e.g., `Thing.reviews`, `Review.thing`) currently require `ModelInstance` fallbacks to avoid `InferInstance` cycles. **Solution:** Use intersection pattern (`type ThingInstance = InferInstance<...> & { reviews?: ReviewInstance[] }`) instead of schema virtuals. See `dal/README.md` "Relation Field Typing" section.
  ...
  - ✅ Make `newRevision()` return type preserve instance methods. Currently `VersionedModelInstance.newRevision()` returns base `VersionedModelInstance<TData, TVirtual>` which loses custom instance methods from `InstanceMethods`. Use polymorphic `this` return type so `review.newRevision()` returns full `ReviewInstance` including `populateUserInfo`, etc.
  ...
  - [x] Explore relation-aware virtual field inference—investigate whether virtual fields for relations (`teams: TeamInstance[]`) can be derived from the relations array + type map, reducing manual `types.virtual().returns<...>()` declarations. **Result:** Intersection pattern is preferred over automatic inference; it's explicit, handles circular references, and avoids schema virtuals entirely.
