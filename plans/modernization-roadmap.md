...
- `greenlock-express@4`, `node-webhooks`, `remote-ac`, `irc-upd`, `promise-limit`, `es6-promise`, `striptags`, and similar utilities – review individually for maintenance status and Node 22 compatibility before upgrades.
...
- [ ] **Build Pipeline Modernization**
  ...
  - [x] Modernize legacy frontend modules (`frontend/*.js`) into true ES modules so Vite can target them directly (drop `entries/` wrappers). *(2025-10-16: Removed `frontend/entries/` wrappers; updated `config/frontend-entries.json` to point directly to `frontend/*.js`; moved style imports and auto-initialization into `libreviews.js` and `editor.js`; all modules now export proper ES module APIs. Build and tests pass.)*
