...
- `filterWhere` is the typed entry point injected into every manifest-derived constructor via `createFilterWhereStatics()` (`dal/lib/filter-where.ts`). Key behaviours:
  ...
  - `FilterWhereBuilder` enforces default revision predicates (`_old_rev_of IS NULL`, `_rev_deleted = false`) unless `.includeStale()` / `.includeDeleted()` is invoked and supports joins/order/limit helpers plus `whereIn`/`orderBy`/`limit` passthroughs to the legacy builder.
