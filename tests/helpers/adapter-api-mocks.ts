...
const overpassResponses = {
  'way(540846325)': buildOverpassElement('Sample Way', {
    en: 'Sample Way EN',
    de: 'Beispielstraße',
  }),
...
function buildOverpassElement(name, localizedNames = {}) {
  ...
  for (const [language, value] of Object.entries(localizedNames)) tags[`name:${language}`] = value;
  return {
    elements: [
      {
        tags,
...
function parseOverpassRequestBody(requestBody) {
  ...
  if (Buffer.isBuffer(requestBody)) return parseOverpassRequestBody(requestBody.toString());
  if (typeof requestBody === 'string') {
    const params = new URLSearchParams(requestBody);
    return params.get('data') || '';
  }
  if (typeof requestBody === 'object' && requestBody.data) return requestBody.data;
  return '';
}
...
function setupOverpassMocks() {
  nock(OVERPASS_HOST)
    ...
    .reply((uri, requestBody) => {
      ...
      if (key) return [200, overpassResponses[key]];
      return [200, { elements: [] }];
    });
}
