 ...
 */
type ParseSubmissionFn = (options?: Parameters<typeof forms.parseSubmission>[1]) => ReturnType<typeof forms.parseSubmission>;
...
class AbstractBREADProvider {
  ...
  protected getResourceErrorHandler: (messageKeyPrefix: string, bodyParam: string) => (error: unknown) => void;
   ...
   */
  constructor(req: ProviderRequest, res: ProviderResponse, next: HandlerNext, options: ProviderOptions = {}) {
    ...
    this.id = typeof resolvedOptions.id === 'string' || resolvedOptions.id === undefined ? resolvedOptions.id as string | undefined : String(resolvedOptions.id);
    ...
    this.renderResourceError = render.resourceError.bind(render, this.req, this.res) as BoundRenderFunction;
    this.renderPermissionError = render.permissionError.bind(render, this.req, this.res) as BoundRenderFunction;
    this.renderSigninRequired = render.signinRequired.bind(render, this.req, this.res) as BoundRenderFunction;
    ...
    if (!actionDef.loadData)
      ...
      Reflect.apply(actionDef.loadData, this, [])
        .then(data => {
          ...
          if (!actionDef.resourcePermissionCheck ||
  ...
  userCan(action: string, data: any): boolean {
    if (typeof data.populateUserInfo === 'function')
  ...
  static bakeRoutes(this: typeof AbstractBREADProvider, resource: string, routes?: BakeRouteMap) {
    ...
    const Provider = this as typeof AbstractBREADProvider & (new (req: ProviderRequest, res: ProviderResponse, next: HandlerNext, options?: ProviderOptions) => AbstractBREADProvider);
    ...
    for (const action in resolvedRoutes) {
      ...
      for (const method of resolvedRoutes[action].methods)
        (router as any)[method.toLowerCase()](resolvedRoutes[action].path,
