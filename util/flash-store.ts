...
export default function flashStore(req: FlashRequest, _res: Response, next: NextFunction): void {
  ...
  const flashImpl = (key: string, value?: unknown): unknown[] => {
    if (!key) throw new TypeError('Flash key is required');
    ...
    if (value === undefined) {
      ...
      if (Object.keys(store).length === 0) Reflect.deleteProperty(session, 'flash');

      return messages;
    }

    if (!session.flash) session.flash = Object.create(null);

    if (!Array.isArray(session.flash[key])) session.flash[key] = [];
